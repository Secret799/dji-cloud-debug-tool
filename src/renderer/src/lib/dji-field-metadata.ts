export interface DjiFieldMetadata {
  path: string
  field: string
  label: string
  type: string
...
export type DjiFieldRow = readonly [
  rawField: string,
  label: string,
  type: string,
  constraint: string,
...
export const DJI_DOCK2_PROPERTY_DOC_URL =
  'https://developer.dji.com/doc/cloud-api-tutorial/cn/api-reference/dock-to-cloud/mqtt/dock/dock2/properties.html'

export const DJI_DOCK2_PROPERTY_DOC_DATE = '2026-01-28'

const DJI_DOCK2_FIELD_ROWS = [
...
const parseEnumValues = (type: string, constraint: string): Readonly<Record<string, string>> | undefined => {
  if ((type !== 'enum_int' && type !== 'enum_string' && type !== 'bool') || !constraint) return undefined

  const normalizedConstraint = constraint.replace(/"(?=-?\d+"\s*:)/g, '","')
  const entries = [...normalizedConstraint.matchAll(/"([^"]+)"\s*:\s*"([^"]*)"/g)]
    .map((match) => [match[1], match[2]])
  return entries.length ? Object.freeze(Object.fromEntries(entries)) : undefined
}

const parseUnit = (constraint: string): string | undefined => {
  const match = constraint.match(/"unit_name"\s*:\s*"([^"]*)"/)
  if (!match) return undefined

  const separatorIndex = match[1].indexOf('/')
  const displayName = (separatorIndex >= 0 ? match[1].slice(0, separatorIndex) : match[1]).trim()
  const symbol = (separatorIndex >= 0 ? match[1].slice(separatorIndex + 1) : '').trim()
  if (symbol && symbol !== '无') return symbol
  return displayName && displayName !== '无' ? displayName : undefined
}

const parseInvalidValues = (description: string): readonly string[] | undefined => {
  const values = [...description.matchAll(/异常值为\s*(-?\d+(?:\.\d+)?)/g)].map((match) => match[1])
  return values.length ? Object.freeze([...new Set(values)]) : undefined
}

export const buildDjiFieldMetadata = (rows: readonly DjiFieldRow[]): DjiFieldMetadata[] => {
  const pathStack: string[] = []
  const metadataStack: DjiFieldMetadata[] = []

  return rows.map(
    ([rawField, label, type, constraint, description, accessMode, pushMode]) => {
      const depth = rawField.match(/^»+/)?.[0].length ?? 0
      const field = rawField.replace(/^»+/, '')
      pathStack[depth] = field
      pathStack.length = depth + 1

      const parent = depth > 0 ? metadataStack[depth - 1] : undefined
      const metadata: DjiFieldMetadata = {
        path: pathStack.join('.'),
        field,
        label,
        type,
        ...
        accessMode: accessMode || parent?.accessMode,
        pushMode: pushMode || parent?.pushMode,
        unit: parseUnit(constraint) ?? (field === 'capacity_percent' ? '%' : undefined),
        enumValues: parseEnumValues(type, constraint),
        invalidValues: parseInvalidValues(description),
      }

      metadataStack[depth] = metadata
      metadataStack.length = depth + 1
      return Object.freeze(metadata)
    },
  )
}

export const DJI_DOCK2_FIELDS = Object.freeze(buildDjiFieldMetadata(DJI_DOCK2_FIELD_ROWS))

const FIELD_BY_PATH = new Map(DJI_DOCK2_FIELDS.map((metadata) => [metadata.path, metadata]))

export const getDjiFieldMetadata = (path: string): DjiFieldMetadata | undefined => {
  const normalizedPath = path
    .split('.')
    .filter((part) => !/^\d+$/.test(part))
    .join('.')
  return FIELD_BY_PATH.get(normalizedPath)
}

export const formatDjiFieldValue = (value: unknown, metadata?: DjiFieldMetadata): string => {
  if (value === undefined || value === null || value === '') return '--'

  const rawValue = String(value)
  if (metadata?.invalidValues?.includes(rawValue)) return `无效值 (${rawValue})`

  const enumLabel = metadata?.enumValues?.[rawValue]
  let formatted: string
  if (enumLabel !== undefined) formatted = `${enumLabel} (${rawValue})`
  else if (metadata?.enumValues) formatted = `未知枚举值 (${rawValue})`
  else if (typeof value === 'boolean') formatted = value ? '是' : '否'
...
export const djiAccessModeLabel = (accessMode?: string): string | undefined => {
  if (accessMode === 'r') return '只读'
  if (accessMode === 'rw') return '可读写'
  return undefined
}

export const djiPushModeLabel = (pushMode?: string): string | undefined => {
  if (pushMode === '0') return 'OSD 定频上报（0.5 Hz）'
  if (pushMode === '1') return '状态变化时上报'
  return undefined
}
